...
class TOP_SAP_Model_observers_SalesPlaced extends Mage_Core_Model_Session_Abstract
{
    ...
    public function orderSAP(Varien_Event_Observer $event_Observer){
        Mage::log("Coordinates of order:",null,"Coordinates.log", true);

        $url = 'https://sandbox.api.sap.com/dq/addressCleanse';
        $request = array(
            "addressInput" => array(
                "street" => $event_Observer->getOrder()->getShippingAddress()->getStreet()[0],
    ...
    public function sendCurlRequest($url, $request){
        $request = json_encode($request);
        $ch = curl_init($url);
        $options = array(
            CURLOPT_RETURNTRANSFER => true,         // return web pageFIXED
